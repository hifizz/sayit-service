...
export interface LlmOptions { apiKey: string; baseUrl: string; model: string; timeoutMs?: number; jsonMode?: 'json_object' | 'json_schema'; fetchImpl?: typeof fetch }
  ...
  async generateJson<T>(input: JsonRequest): Promise<T> {
    ...
    try {
      const response = await (this.options.fetchImpl ?? fetch)(this.options.baseUrl.replace(/\/$/,'')+'/chat/completions', {
        ...
        body:JSON.stringify({ model:this.options.model, temperature:input.temperature ?? 0, max_tokens:8192,
          messages:[{role:'system',content:input.system+'\nRequired JSON schema: '+JSON.stringify(input.schema)},{role:'user',content:input.user}],
          response_format:structured ? {type:'json_schema',json_schema:{name:input.schemaName,strict:true,schema:input.schema}} : {type:'json_object'} })
      });
      if (!response.ok) throw upstreamError(response.status);
      const body = await response.json() as {choices?: Array<{finish_reason?:string;message?:{content?:unknown}}>};
      const choice = body.choices?.[0]; const content = choice?.message?.content;
